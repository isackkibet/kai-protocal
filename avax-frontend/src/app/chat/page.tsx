...
export default function AVAXChatPage() {
  ...
  return (
    <main style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: '#0a0a0c' }}>
      ...
      {/* HEADER */}
      <div style={{ padding: '44px 16px 12px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0, borderBottom: '1px solid rgba(16,185,129,0.12)', background: 'rgba(10,10,12,0.9)', backdropFilter: 'blur(16px)' }}>
        <Link href="/" style={{ color: 'rgba(255,255,255,0.5)', display: 'flex', alignItems: 'center' }}>
          <ChevronLeft size={20} />
        </Link>
        <div className="float" style={{ position: 'relative' }}>
          <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg,#10b981,#064e3b)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 16px rgba(16,185,129,0.5)' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button
            ...
            style={{ width: 32, height: 32, borderRadius: 8, background: voiceOn ? 'rgba(16,185,129,0.18)' : 'rgba(255,255,255,0.06)', border: voiceOn ? '1px solid rgba(16,185,129,0.45)' : '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
          >
            {voiceOn ? <Volume2 size={14} color="#10b981" /> : <VolumeX size={14} color="rgba(255,255,255,0.4)" />}
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '10px 16px 0', display: 'flex', gap: 6, overflowX: 'auto', flexShrink: 0, scrollbarWidth: 'none' }}>
        {QUICK_PROMPTS.map(p => (
          <button key={p} onClick={() => send(p)} style={{ flexShrink: 0, padding: '5px 12px', borderRadius: 20, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.22)', color: 'rgba(255,255,255,0.75)', fontSize: 10, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
            {p}
          </button>
        ))}
      </div>
      ...
      {/* INPUT BAR */}
      <div style={{ padding: '12px 16px', background: 'rgba(10,10,12,0.92)', backdropFilter: 'blur(16px)', borderTop: '1px solid rgba(16,185,129,0.12)', display: 'flex', gap: 10, alignItems: 'center', paddingBottom: 'max(12px, env(safe-area-inset-bottom))' }}>
        <textarea
          value={input}
          ...
          rows={1}
          style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 14, padding: '10px 14px', color: '#fff', fontSize: 13, resize: 'none', outline: 'none', fontFamily: 'inherit', lineHeight: 1.5, maxHeight: 100 }}
        />
        <button
          onClick={() => send()}
          disabled={!input.trim() || loading}
          style={{ width: 42, height: 42, borderRadius: '50%', background: input.trim() && !loading ? 'linear-gradient(135deg,#10b981,#064e3b)' : 'rgba(255,255,255,0.06)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: input.trim() && !loading ? 'pointer' : 'not-allowed', boxShadow: input.trim() && !loading ? '0 0 14px rgba(16,185,129,0.45)' : 'none', transition: 'all 0.2s', flexShrink: 0 }}
        >
