 ...
 * Every "Withdraw" sends tokens back from TREASURY to the user.
 *
 * Token addresses come from deployedAddresses.json.
 */
...
const ALL_PRODUCTS = [...PRODUCTS, ...INSURANCE, ...COMMUNITY];
...
export default function ProductsPage() {
  ...
  const handleDeposit = async (product: Product) => {
    ...
    try {
      ...
      setStatusMsg(`Confirming ${FEE_AVAX} AVAX policy fee in MetaMask…`);
      const feeTx = await sendTransactionAsync({
        to: TREASURY,
        value: parseEther(FEE_AVAX),
      });
      setTxUrl(`https://testnet.snowtrace.io/tx/${feeTx}`);

      setStatusMsg(`Confirming ${amt} ${product.tokenSymbol} transfer in MetaMask…`);
      const cleanAmtStr = stakeAmt.trim();
      const tokenUnits = parseUnits(cleanAmtStr, token.decimals);
      ...
      setStatusMsg(
        `✅ Deposited ${amt} ${product.tokenSymbol} — KAI Product policy active! ` +
      ...
      if (/user rejected|user denied|rejected the request/i.test(msg)) {
        setStatusMsg("⚠️ Transaction signature canceled in MetaMask.");
      } else {
  ...
  const handleWithdraw = async (product: Product) => {
    ...
    setIsLoading(true);
    setStatusMsg(`Initiating withdrawal — paying ${FEE_AVAX} AVAX release fee in MetaMask…`);
    setTxUrl(null);
    ...
    try {
      ...
      setStatusMsg(`✅ Release fee paid · ${invested} ${product.tokenSymbol} product funds released! Tx: ${feeTx.slice(0, 14)}…`);
      ...
      const msg = err instanceof Error ? err.message : String(err);
      if (/user rejected|user denied|rejected the request/i.test(msg)) {
        setStatusMsg("⚠️ Withdrawal transaction canceled in MetaMask.");
      } else {
        setStatusMsg(`❌ ${msg.slice(0, 140)}`);
      }
    } finally {
  ...
  const currentList      = activeTab === "Products" ? PRODUCTS : activeTab === "Insurance" ? INSURANCE : COMMUNITY;
    ...
    <main style={{ padding: "16px 16px 100px", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ paddingTop: 32, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 900, color: "#fff", margin: 0 }}>🏛️ Products &amp; Insurance</h1>
              ...
              {isActive && (
                <div style={{ padding: "0 16px 16px" }}>
                  ...
                  <div style={{
                    ...
                    borderRadius: 10, padding: "10px 12px", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14,
                  }}>
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); toggleCondition(product.id); }}
                      style={{
                        background: status === "UNLOCKED" ? "rgba(34,197,94,0.18)" : "rgba(255,255,255,0.08)",
                        border: `1px solid ${status === "UNLOCKED" ? "rgba(34,197,94,0.4)" : "rgba(255,255,255,0.15)"}`,
                        borderRadius: 8, padding: "5px 10px", color: "#fff", fontSize: 10, fontWeight: 700, cursor: "pointer",
                      }}
                      title="Click to toggle release condition"
                    >
